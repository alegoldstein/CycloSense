...
#include <string.h>
#include <stdlib.h>
#include <math.h>               // ← needed for fabs() if called from .c side
...
static bool parse_RMC(char **f, int n, GPS_data *out) {
    ...
    out->speed_knots = (float)atof(f[7]);
    out->heading_deg = (float)atof(f[8]);
    return true;
}
...
static bool parse_VTG(char **f, int n, GPS_data *out) {
    if (n < 8) return false;
    out->heading_deg = (float)atof(f[1]);
    out->speed_knots = (float)atof(f[5]);
    return true;
}
